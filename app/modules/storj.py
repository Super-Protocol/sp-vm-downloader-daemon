...
class StorJ:
    ...
    def _download_file(self, bucket: str, prefix: str, filename: str, dst: str) -> None:
        ...
        if res.returncode != 0:
            raise Exception(f"failed to download file {filename}, command: {cmd}, result: {res.stdout}")
    ...
    def download_release_files(self, release: models.Release) -> tempfile.TemporaryDirectory:
        self.logger.info(f"downloading release: {release.name}")

        artifacts = release.artifacts

        temp_dir = tempfile.TemporaryDirectory(prefix="sp_downloader_", suffix=f"_{release.name}")
